// leave off @2x/@3x
const MiscImages = {
  'Items': require('./images/misc/ItemIcon051.png'),
  'Quests': require('./images/misc/ItemIcon058.png'),
  'Charms': require('./images/misc/charm.png'),
  'Decorations': require('./images/misc/ItemIcon017.png'),
  'Skills': require('./images/misc/ItemIcon026.png'),
  'Maps': require('./images/misc/ItemIcon027.png'),
  'About': require('./images/misc/ItemIcon058f.png'),
  'Remove Ads': require('./images/misc/ItemIcon060e.png'),
  'Theme': require('./images/misc/ItemIcon060e.png'),
  'Combination List': require('./images/misc/ItemIcon026c.png'),
  'android-more': require('./images/misc/android-more.png'),
  'ios-more': require('./images/misc/ios7-more.png'),
  'ios-back': require('./images/misc/ios7-arrow-left.png'),
  'android-back': require('./images/misc/android-arrow-back.png'),
  plusblack: require('./images/misc/plus-roundw.png'),
  pluswhite: require('./images/misc/plus-round.png'),
  plus: require('./images/misc/plus-roundw.png'),
  favorite: require('./images/misc/favorites.png'),
  Favorites: require('./images/misc/favorites2.png'),
  unfavorite: require('./images/misc/unfavorite.png'),
  'Rate Our App!': require('./images/misc/VillageIcon08.png'),
  'Hunter Tools': require('./images/misc/placeholder_mantle.png'),
  'Set Builder': require('./images/misc/chest.png'),
  'Endemic Life': require('./images/misc/ItemIcon016a.png'),
};

export default MiscImages;
